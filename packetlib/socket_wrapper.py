...
def send_check_DAQ_gen_params(socket, 
                              addr, 
                              port, 
                              asic_num, 
                              fpga_addr, 
                              ...
                              asic0_collection=0x00, asic1_collection=0x00, asic2_collection=0x00, asic3_collection=0x00, 
...
def get_proto_monitor(socket, addr, port, asic_num, fpga_addr, verbose=False):
    header = 0xA0
    data_packet = pack_data_req_proto_monitor(header, fpga_addr)
    if verbose:
        print("\033[32mSending data packet:\033[0m")
        for i in range(0, len(data_packet), 8):
            print(" ".join(f"{b:02X}" for b in data_packet[i:i+8]))
    ...
    if verbose:
        print("\033[32mReceived data packet:\033[0m")
    received_data, addr = socket.recvfrom(8196)
    unpacked_data = unpack_data_rpy_proto_monitor(received_data)
    max_key_length = max(len(key) for key in unpacked_data)
    if verbose:
        for key in unpacked_data:
            print(f"{key:<{max_key_length}} : {hex(unpacked_data[key])}")
...
def set_req_proto_monitor_off(socket, addr, port, _fpga, verbose=True):
    header = 0xA0
    data_packet = pack_req_proto_monitor_off(header, _fpga)
    if verbose:
        print("\033[32mSending data packet:\033[0m")
        for i in range(0, len(data_packet), 8):
            print(" ".join(f"{b:02X}" for b in data_packet[i:i+8]))
...
def set_req_proto_monitor_on(socket, addr, port, _fpga, verbose=True):
    header = 0xA0
    data_packet = pack_req_proto_monitor_on(header, _fpga)
    if verbose:
        print("\033[32mSending data packet:\033[0m")
        for i in range(0, len(data_packet), 8):
            print(" ".join(f"{b:02X}" for b in data_packet[i:i+8]))
...
def get_monitor_data(socket, addr, port, fpga_addr, verbose=True):
    header = 0xA0
    data_packet = pack_data_req_get_debug_data(header, fpga_addr)
    if verbose:
        print("\033[32mSending data packet:\033[0m")
